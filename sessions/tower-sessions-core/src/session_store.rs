...
pub trait SessionStore: Debug + Send + Sync + 'static {
    ...
    async fn create(&self, data: &SessionData) -> Result<Id>;

    /// Saves the provided session record to the store.
    ///
    /// This method is only intended for updating the state of an existing session.
    async fn save(&self, id: &Id, data: &SessionData) -> Result<()>;
    ...
    /// returned.
    async fn load(&self, session_id: &Id) -> Result<Option<SessionData>>;
...
//         self.cache.create(data).await?;
//         Ok(())
//     }

//     async fn save(&self, data: &SessionData) -> Result<()> {
//         let store_save_fut = self.store.save(data);
//         let cache_save_fut = self.cache.save(data);
